...
func genClientMethod(gen *protogen.Plugin, file *protogen.File, g *protogen.GeneratedFile, method *protogen.Method, index int) {
	...
	sname := fmt.Sprintf("/%s/%s", serviceName(service), method.Desc.Name())
...
var serviceAliasRe = regexp.MustCompile(`@alias (\S+)`)

func serviceName(service *protogen.Service) protoreflect.FullName {
	sub := serviceAliasRe.FindStringSubmatch(service.Comments.Leading.String())
	if len(sub) != 2 {
		return service.Desc.FullName()
	}

	return protoreflect.FullName(sub[1])
}
...
func genServerMethod(gen *protogen.Plugin, file *protogen.File, g *protogen.GeneratedFile, method *protogen.Method) string {
	...
	if !method.Desc.IsStreamingClient() && !method.Desc.IsStreamingServer() {
		poolIName := fmt.Sprintf("With%s%sRequestPool", service.GoName, method.GoName)
		g.P("type ", poolIName, " interface {")
		g.P("Get", method.Input.GoIdent.GoName, "() (*", method.Input.GoIdent, ", error)")
		...
		g.P("return nil, err")
		g.P("}")
		g.P("defer p.Put", method.Input.GoIdent.GoName, "(in)")
		g.P("} else {")
